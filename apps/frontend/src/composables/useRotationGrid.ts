...
export type Column = { key: string; label: string; isMimaori: boolean; pointId: number; pointName: string }
...
export function useRotationGrid(
  ...
  const columns = computed((): Column[] => {
    ...
    for (const p of points.value) {
      cols.push({ key: `point-${p.id}-1`, label: '①', isMimaori: false, pointId: p.id, pointName: p.name })
      cols.push({ key: `point-${p.id}-2`, label: '②', isMimaori: false, pointId: p.id, pointName: p.name })
      cols.push({ key: `mimaori-${p.id}`, label: '見守り', isMimaori: true, pointId: p.id, pointName: p.name })
    }
    return cols
  })
