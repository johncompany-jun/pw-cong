...
async function fetchSchedules(page: number) {
  ...
  try {
    ...
    if (!auth.user?.isAdmin && auth.user?.id != null) {
      params.set('mcUserId', String(auth.user.id))
    }
    const res = await api.get<{ data: ScheduleItem[]; pagination: PaginationData }>(`/schedules?${params.toString()}`)
